...
public protocol CaptureAPIClient: Sendable {
    ...
    /// `GET /api/projects/:project_id/documents` — documents attached
    /// to a project. Per Genie's 2026-05-23 upload-traceability answer,
    /// documents are project-scoped reference materials (PDFs, prior
    /// specs, architecture diagrams generated by the AI pipeline),
    /// not session artifacts. Server-side rows live in `app.uploads`
    /// with `kind = 'document'`, returned in the same shape as
    /// `CaptureSession.uploads`.
    ///
    /// Pagination mirrors `listProjectCaptureSessions`: newest-first
    /// by `uploaded_at`, `limit` caps page size, `before` cursors
    /// off the oldest seen `uploaded_at`.
...
public actor LiveCaptureAPIClient: CaptureAPIClient {
    ...
    public func listProjectDocuments(
        ...
        components.queryItems = items
        let query = components.percentEncodedQuery ?? ""
        let path = "/api/projects/\(projectID)/documents" + (query.isEmpty ? "" : "?\(query)")

        struct ListResponse: Decodable {
            let documents: [CaptureUpload]
        }
        let response: ListResponse = try await send(
            workspaceID: workspaceID,
            method: .get,
